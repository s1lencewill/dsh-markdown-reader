/**
 * @s1lencewill/dsh-markdown-reader — host half: the workspace-gated read-only
 * routes (/md-reader/read, /md-reader/stat, /md-reader/raw) plus the vendored-asset route
 ...
 * @module @s1lencewill/dsh-markdown-reader
 */
...
export const MARKDOWN_READER_GUIDANCE = '本机已安装 dsh-markdown-reader 插件（DSH Web GUI 的全屏 Markdown 阅读器）：工作区内的 .md/.markdown/.mdx 文件可在全屏阅读浮层中打开，支持 GFM 语法（表格/任务列表/删除线等）、大纲目录与滚动定位、KaTeX 数学公式、Mermaid 图表（离线降级展示）、相对路径图片与文档内 .md 跳转；并支持外部文件变更提示/自动重载、阅读进度与预计时间、图片放大下载、代码复制折叠行号、宽表格和长章节折叠。入口：右下角悬浮按钮（打开最近文件/路径输入）、Ctrl+Shift+M 快捷键、右侧预览面板工具栏的「阅读模式」按钮。用户提到「阅读器 / 阅读模式 / markdown 阅读 / 阅读 xx.md」时即指本插件。数据经宿主端 /md-reader/* 路由读取，仅限已注册工作区，只读。'
...
function createWorkspaceGate(ctx) {
  return async (root) => {
    ...
    for (const workspace of workspaces) {
      ...
      let registered
      try {
        registered = await realpath(workspace.path)
      } catch {
        continue
      }
      if (isPathInside(registered, canonical)) return { ok: true, canonical }
    }
...
async function serveRaw(gate, url, res) {
  ...
  try {
    ...
    const mime = RAW_MIME[extname(resolved.real).toLowerCase()] ?? 'application/octet-stream'
    const headers = {
      'content-type': mime,
...
/**
 * POST /md-reader/stat: return lightweight file metadata for change polling.
 * It intentionally shares the same workspace/type gate as /read and never
 * returns file content.
 */
async function handleStat(gate, payload, res) {
  const root = strField(payload, 'root')
  const path = strField(payload, 'path')
  if (root === null || path === null) {
    json(res, FAIL('malformed-request', 'missing root or path'), 400)
  ...
  if (!MD_EXT.has(extname(resolved.real).toLowerCase())) {
    json(res, FAIL('unsupported-type', 'not a markdown file'), 415)
    return
  }
  try {
    ...
    json(res, { ok: true, value: { mtime: info.mtimeMs, size: info.size } })
  } catch {
    json(res, FAIL('not-found', 'file does not exist'), 404)
  }
}
...
function registerRoutes(ctx, gate) {
  const handler = async (req, res) => {
    ...
    if (pathname === '/md-reader/stat') {
      await handleStat(gate, payload, res)
      return
    }
    res.writeHead(404)
    res.end()
