...
const STRINGS = {
  zh: {
    ...
    reloadFailed: '刷新失败',
    print: '打印 / PDF',
    printPreparing: '正在准备打印…',
    printFailed: '无法打开打印，请重试或检查浏览器的打印权限。',
    printTruncated: '注意：原文件超过读取上限，以下仅为已载入的部分内容。',
    printFallback: '部分公式或图表未完成渲染，以下保留其源码。',
    printImageMissing: '图片未能加载',
  },
  en: {
    panelBtn: 'Read',
    ...
    printFailed: 'Could not open printing. Retry or check browser printing permissions.',
    printTruncated: 'Warning: the file exceeds the read limit. Only the loaded portion is included.',
    printFallback: 'Some formulas or diagrams are not rendered; their source is included below.',
    printImageMissing: 'Image could not be loaded',
  },
}
...
/** Isolated print layout. Never import shell styles or print the DSH page. */
const PRINT_CSS = `
@page { size: auto; margin: 16mm 14mm; }
* { box-sizing: border-box; }
html, body { margin: 0; padding: 0; background: #fff; color: #111; }
body { font: 11pt/1.6 -apple-system, BlinkMacSystemFont, "Segoe UI", "Microsoft YaHei", sans-serif; }
.mr-print-heading { padding-bottom: 6mm; margin-bottom: 6mm; border-bottom: 1px solid #bbb; font-size: 10pt; color: #555; overflow-wrap: anywhere; }
.mr-print-warning { padding: 3mm; border: 1px solid #aaa; margin-bottom: 5mm; font-size: 10pt; }
.mr-doc { width: 100%; overflow-wrap: anywhere; }
h1, h2, h3, h4, h5, h6 { line-height: 1.3; break-after: avoid; page-break-after: avoid; margin: 1.2em 0 .6em; }
h1 { font-size: 22pt; } h2 { font-size: 17pt; } h3 { font-size: 14pt; }
p, li { orphans: 3; widows: 3; }
p, ul, ol, blockquote { margin: 0 0 .8em; }
blockquote { margin-left: 0; padding-left: 4mm; border-left: 1mm solid #ccc; color: #444; }
a { color: #174f86; text-decoration: underline; }
code, pre { font-family: ui-monospace, SFMono-Regular, Consolas, monospace; font-size: 9pt; }
code { white-space: pre-wrap; overflow-wrap: anywhere; }
pre { white-space: pre-wrap; overflow-wrap: anywhere; padding: 3mm; margin: 0 0 4mm; border: 1px solid #ccc; break-inside: auto; }
pre code { display: block; }
.mr-code-wrap, .mr-table-wrap { overflow: visible; max-height: none; }
table { width: 100%; table-layout: fixed; border-collapse: collapse; margin-bottom: 4mm; font-size: 9pt; }
th, td { border: 1px solid #aaa; padding: 2mm; overflow-wrap: anywhere; vertical-align: top; }
th { background: #eee; color: #111; position: static; }
thead { display: table-header-group; } tfoot { display: table-footer-group; }
tr { break-inside: avoid; page-break-inside: avoid; }
img { display: inline-block; max-width: 100%; max-height: 230mm; object-fit: contain; height: auto; break-inside: avoid; }
.mr-print-image-missing { display: block; padding: 3mm; border: 1px dashed #999; font-size: 10pt; }
.mr-math-display { display: block; text-align: center; margin: 4mm 0; overflow: visible; break-inside: avoid; }
.katex-display { margin: 0; }
.mr-mermaid-render { text-align: center; margin: 4mm 0; padding: 3mm; break-inside: avoid; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
.mr-mermaid-render svg { max-width: 100%; max-height: 230mm; height: auto; }
.mr-task { list-style: none; } .mr-task-row { display: flex; gap: 2mm; align-items: baseline; }
.mr-task-row input { flex: none; }
hr { border: 0; border-top: 1px solid #bbb; margin: 5mm 0; }
button, .mr-code-toolbar, .mr-section-toggle { display: none !important; }
`

/** Bounded async work with immediate cancellation and no dangling timers. */
function waitForPrintTask(task, signal, timeoutMs = 12000) {
  return new Promise((resolve, reject) => {
    ...
    const abort = () => {
      const error = new Error('Print cancelled')
      error.name = 'AbortError'
      finish(error)
    }
    // Attach both handlers even if already cancelled (consume late rejection).
...
/** Clone only the sanitized article; expand the clone without touching the reader. */
function createPrintSnapshot(article) {
  const clone = article.cloneNode(true)
  clone.className = 'mr-doc'
  clone.removeAttribute('style')
  const originals = Array.from(article.querySelectorAll('pre'))
  const copies = Array.from(clone.querySelectorAll('pre'))
  copies.forEach((pre, index) => {
    if (pre.classList.contains('mr-mermaid') && pre.nextElementSibling?.classList.contains('mr-mermaid-render')) {
      pre.remove() // Print the finished SVG, not the duplicate Mermaid source.
      return
    }
    const code = pre.querySelector('code')
    if (code !== null) code.textContent = originals[index].__mrCodeText ?? originals[index].querySelector('code')?.textContent ?? ''
    pre.removeAttribute('style')
  })
  for (const el of clone.querySelectorAll('button, .mr-code-toolbar, script, iframe, object, embed')) el.remove()
  for (const el of clone.querySelectorAll('[hidden]')) el.removeAttribute('hidden')
  for (const el of clone.querySelectorAll('.mr-code-collapsed')) el.classList.remove('mr-code-collapsed')
  for (const el of clone.querySelectorAll('details')) el.setAttribute('open', '')
  const originalImages = Array.from(article.querySelectorAll('img'))
  Array.from(clone.querySelectorAll('img')).forEach((img, index) => {
    img.src = originalImages[index].currentSrc || originalImages[index].src
    img.loading = 'eager'
    img.removeAttribute('tabindex')
  })
  // Preserve contrast of already-rendered dark Mermaid diagrams on white paper.
  const diagrams = Array.from(article.querySelectorAll('.mr-mermaid-render'))
  Array.from(clone.querySelectorAll('.mr-mermaid-render')).forEach((el, index) => {
    el.style.backgroundColor = article.ownerDocument.defaultView.getComputedStyle(diagrams[index]).backgroundColor
  })
  return clone
}

/** Scale unbreakable math to its cell/page width after fonts finish loading. */
function fitPrintMath(doc) {
  for (const el of doc.querySelectorAll('.katex')) {
    const parent = el.closest('td, th, .mr-math-display') ?? el.parentElement
    ...
    if (available > 0 && width > available) {
      ...
      el.style.fontSize = `${size * available / width}px`
    }
  }
}

/** Build a same-origin, script-disabled print frame and wait for the dialog to close. */
async function printReaderDocument(article, meta, signal) {
  if (signal?.aborted) return
  const owner = article.ownerDocument
  const previousFocus = owner.activeElement
  const snapshot = createPrintSnapshot(article)
  const frame = owner.createElement('iframe')
  frame.className = 'mr-print-frame'
  frame.title = t('print')
  frame.setAttribute('aria-hidden', 'true')
  frame.setAttribute('sandbox', 'allow-same-origin allow-modals')
  frame.tabIndex = -1
  // Kept laid out for fonts/images, but never displayed over the live reader.
  frame.style.cssText = 'position:fixed;left:-10000px;top:0;width:182mm;height:1px;border:0;pointer-events:none;'
  ...
  try {
    ...
    if (doc === null || win === null || typeof win.print !== 'function') throw new Error('Printing unavailable')
    doc.documentElement.lang = currentLang
    doc.title = meta.title || basename(meta.path || 'document.md')
    ...
    heading.className = 'mr-print-heading'
    heading.textContent = meta.path || meta.title || ''
    doc.body.appendChild(heading)
    const warning = (message) => {
      const note = doc.createElement('p')
      note.className = 'mr-print-warning'
      note.textContent = message
      doc.body.insertBefore(note, doc.body.querySelector('.mr-doc'))
    }
    if (meta.truncated) warning(t('printTruncated'))
    if (snapshot.querySelector('.mr-math-pending, pre.mr-mermaid') !== null) warning(t('printFallback'))
    doc.body.appendChild(doc.importNode(snapshot, true))

    // Listen before setting URLs/appending resources to also handle cached loads.
    const resource = (element) => new Promise((resolve) => {
    ...
    const imageLoads = Array.from(doc.images).map((img) => img.complete ? Promise.resolve(img.naturalWidth > 0) : resource(img))
    if (snapshot.querySelector('.katex') !== null) {
      const css = doc.createElement('link')
      css.rel = 'stylesheet'
      const loaded = resource(css)
      css.href = new URL(`${ASSET_BASE}/katex/katex.min.css`, owner.baseURI).href
      doc.head.appendChild(css)
      if (!(await waitForPrintTask(loaded, signal))) throw new Error('KaTeX print stylesheet unavailable')
    }
    // Force font discovery before awaiting FontFaceSet.ready.
    void doc.body.offsetHeight
    const ready = Promise.all([Promise.all(imageLoads), doc.fonts?.ready ?? Promise.resolve()])
    if ((await waitForPrintTask(ready, signal)) === false) throw new Error('Print resources timed out')
    for (const img of Array.from(doc.images)) {
      if (img.naturalWidth > 0) continue
      const placeholder = doc.createElement('span')
      placeholder.className = 'mr-print-image-missing'
      placeholder.textContent = `${t('printImageMissing')}: ${img.alt || basename(new URL(img.src, owner.baseURI).pathname)}`
...
function ReaderApp({ onReady }) {
  ...
  const [printing, setPrinting] = ReactLib.useState(false)
  const [printError, setPrintError] = ReactLib.useState('')
  ...
  const printJobRef = ReactLib.useRef(null)
  const diagramJobRef = ReactLib.useRef(Promise.resolve())
  ...
  const startPrint = ReactLib.useCallback(async () => {
    if (!open || status !== 'ready' || doc === null || printJobRef.current !== null) return
    const job = new AbortController()
    printJobRef.current = job
    setPrinting(true)
    setPrintError('')
    try {
      // Wait for in-flight diagrams, but preserve source as an offline fallback.
      if (scrollRef.current?.querySelector('pre.mr-mermaid')) {
        await waitForPrintTask(diagramJobRef.current, job.signal, 6000)
      }
      const article = scrollRef.current?.querySelector('.mr-doc')
      if (job.signal.aborted || article === null || article === undefined) return
      await printReaderDocument(article, doc, job.signal)
    } catch (error) {
      if (!job.signal.aborted) {
        console.warn('[dsh-markdown-reader] print failed:', error)
        setPrintError(t('printFailed'))
      }
    } finally {
      if (printJobRef.current === job) {
  ...
  ReactLib.useEffect(() => () => { printJobRef.current?.abort() }, [])

  // Only intercept browser printing while a ready reader is actually open.
  ReactLib.useEffect(() => {
    if (!open || status !== 'ready' || doc === null) return undefined
    const onPrintKey = (event) => {
  ...
  ReactLib.useEffect(() => {
    ...
    let checking = false
    const check = async () => {
      if (checking || stopped || printJobRef.current !== null || (typeof document !== 'undefined' && document.visibilityState === 'hidden')) return
      checking = true
      const res = await apiStat(activeRoot, doc.path)
      checking = false
      if (stopped || printJobRef.current !== null) return
      if (!res.ok) {
  ...
  ReactLib.useEffect(() => {
    ...
    diagramJobRef.current = renderMermaidIn(scroller, t('mermaidFallback'))
  ...
  ReactLib.useEffect(() => {
    ...
    diagramJobRef.current = refreshMermaid(scrollRef.current, t('mermaidFallback'))
  }, [darkTick, status, doc])
  ...
  const header = h('div', { className: 'mr-header' },
    ...
    status === 'ready' && h('button', {
      type: 'button', className: 'mr-btn',
      title: `${t('print')} (Ctrl/Cmd+P)`,
      'aria-label': t('print'),
      disabled: printing,
  ...
  return h('div', { id: 'mr-root' },
    ...
    h('div', {
      ...
      liveBanner,
      printError !== '' && h('div', { className: 'mr-live-banner', role: 'alert' },
        h('span', { className: 'mr-live-banner-text' }, printError),
        h('button', { type: 'button', className: 'mr-btn', onClick: () => setPrintError('') }, t('close')),
      ),
