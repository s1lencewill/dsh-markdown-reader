'use strict'
/* ============================================================================
 * @linxin666/dsh-markdown-reader — browser half (single-file bundle)
 *
...
const PKG_ID = '@linxin666/dsh-markdown-reader'
  ...
  while (child !== null) {
    ...
    if (child.nodeType === 3) {
      ...
      if (tag === 'a') {
        ...
        if (src !== null && (src.startsWith('/md-reader/raw') || /^https?:/i.test(src))) {
...
function rawUrl(root, path, suffix) {
  return `${RAW_ROUTE}?root=${encodeURIComponent(root)}&path=${encodeURIComponent(path)}${suffix ?? ''}`
}
...
function resolveRelativeLink(fromMd, href) {
  ...
  const pathPart = hIdx === -1 ? trimmed : trimmed.slice(0, hIdx)
  const fragment = hIdx === -1 ? null : trimmed.slice(hIdx + 1)
...
/** Reader UI prefs per root (font size, outline visibility). */
function getUi(root) {
...
.mr-doc pre code { background: none; padding: 0; font-size: 13px; line-height: 1.55; }
.mr-doc table {
  border-collapse: collapse; margin: 0 0 16px; width: max-content; max-width: 100%;
  display: block; overflow-x: auto;
}
.mr-doc th, .mr-doc td { border: 1px solid var(--mr-border); padding: 6px 13px; }
.mr-doc th { background: var(--mr-table-head); font-weight: 600; }
.mr-doc img { max-width: 100%; border-radius: 6px; }
...
function ReaderApp({ onReady }) {
  ...
  const openFile = ReactLib.useCallback(async (path, fragment) => {
  ...
  /** Click delegation for relative links (in-document .md navigation). */
    ...
    ctx.effect(() => {
      ...
      try {
        ...
        return () => {
          ...
          console.log(`[dsh-markdown-reader] 生命周期 dispose #${generation} called (${alive() ? 'live' : 'stale → skipped'}, current gen ${applyGeneration})`)
          if (!alive()) return
          try {
            if (window.dshMarkdownReader !== null && typeof window.dshMarkdownReader === 'object' && window.dshMarkdownReader.__mrGen === generation) {
